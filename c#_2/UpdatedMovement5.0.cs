...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float airSpeed = 4f;     // Air speed value
    public float dashSpeed = 15f;   // Dash speed value
    ...
    // List to track movement history
    ...
    void Update()
    {
        ...
        // Store movement direction in history if moving
        if (moveDirection != 0)
        {
            movementHistory.Add(new Vector2(moveDirection, 0)); // Store the movement direction
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        MovePlayer(moveDirection, isGrounded ? currentSpeed : airSpeed);
    }
    ...
    // Function to perform air dash
    void PerformAirDash()
    {
        Vector2 dashDirection = Vector2.zero;

        if (Input.GetKey(KeyCode.W)) dashDirection.y += 1; // Up
        if (Input.GetKey(KeyCode.S)) dashDirection.y -= 1; // Down
        if (Input.GetKey(KeyCode.A)) dashDirection.x -= 1; // Left
        if (Input.GetKey(KeyCode.D)) dashDirection.x += 1; // Right

        // Normalize dash direction to avoid faster diagonal dashes
        if (dashDirection != Vector2.zero)
        {
            // Normalize the dash direction
            dashDirection.Normalize();

            // Set horizontal speed for left and right dashes
            if (dashDirection.x != 0)
            {
                rb.velocity = new Vector2(dashDirection.x * dashSpeed, 0); // Only apply horizontal speed
            }
            else
            {
                rb.velocity = new Vector2(0, dashDirection.y * dashSpeed); // Apply vertical speed for up/down dashes
            }
        }
    }
